// const socket = io();
const socket = io("http://52.6.55.138:3221");
...
const isLeader = isLeaderParam === 'true';// ตรวจสอบและแปลงค่า isLeader
  ...
  // Listener สำหรับข้อความใหม่
socket.on('updateMessage', ({ playerId, message, playerName }) => {
  ...
  newMessage.innerHTML = `<strong><span class="player-name-chat">${playerName}:</span></strong> ${message}`;
  chat.appendChild(newMessage);

  // Auto scroll to the latest message
  chat.scrollTop = chat.scrollHeight;

  // Update player message box (for displaying recent message under their profile)
    ...
    textBox.addEventListener('keydown', (event) => {
      if (event.key === 'Enter' && textBox.value.trim()) {
        socket.emit('sendMessage', {
          ...
          playerName: playerName // เพิ่ม playerName ตรงนี้
        });
...
function assignWordsToPlayers(gameData) {
  ...
  if (currentPlayer) {
    ...
    wordDisplay.className= 'your-word-display';
    const wordbox = document.createElement('div');
    const yourword = document.createElement('div');
    yourword.innerHTML= `<strong><span class="yourword">Your Word:</span></strong>`
    wordbox.className = "word-box";

    wordContainer.appendChild(wordbox);
    wordbox.appendChild(yourword);
    wordbox.appendChild(wordDisplay);
...
socket.on('startVoting', ({ duration, players }) => {
  ...
  mess.innerHTML = `<strong><span class="votespy">     Vote Spy!</span></strong>`;
...
window.onload = function () {
  ...
  const n = document.createElement('div');
    n.classList.add('roomname');
    n.innerHTML = `<strong><span class="player-name-chat">ROOM: </span></strong>${roomName}`;
...
// เมื่อเกมจบ
socket.on('gameOver', ({ result, room, spyName }) => {
  setTimeout(() => {
    const r = document.createElement('div');
    r.className = 'result';
    if (result === "win") {
      r.innerText = `Players win! Spy was ${spyName}`;
    } else {
      r.innerText = `Spy wins! The Spy was ${spyName}`;
    }

    // ทำให้ข้อความแสดงกลางหน้าจอ
  ...
  // เปิดใช้งานปุ่ม Start Game หลังจบเกม
  let startButton = document.getElementById('start-game-button');
  if (startButton) {
    startButton.disabled = false;
  }
