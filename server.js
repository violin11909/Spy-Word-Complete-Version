...
const io = socketIo(server, {
  cors: {
    origin: "*", // or your frontend domain for security
    methods: ["GET", "POST"],
  },
});
...
server.listen(PORT, "0.0.0.0", () => {
  console.log(`Server running on port ${PORT} and ready at http://localhost:${PORT}`);
});
...
io.on('connection', (socket) => {
  ...
  socket.on('createRoom', (roomData) => {
    ...
    if (rooms[roomName]) {
      ...
      socket.emit('roomJoined', { room: roomName, name: playerName, profile: profilePath ,isLeader: true}); // ส่งเฉพาะผู้สร้างห้อง
    }
  });
  ...
  socket.on('leaveRoom', () => {
    for (let roomName in rooms) {
      if (rooms[roomName].players[socket.id]) {
        ...
        setTimeout(() => {
          ...
          if (wasLeader) {
            ...
            if (remainingPlayers.length > 0) {
                // ตรวจสอบว่ามี leader ในห้องอยู่แล้วหรือไม่
                const hasLeader = remainingPlayers.some(playerId => rooms[roomName].players[playerId].isLeader);

                if (!hasLeader) {
                    // หากไม่มี leader ให้สุ่มหัวห้องใหม่
                    const newLeaderId = remainingPlayers[Math.floor(Math.random() * remainingPlayers.length)];
                    rooms[roomName].players[newLeaderId].isLeader = true;
                    io.to(roomName).emit('leaderChanged', { leaderId: newLeaderId });
                }
            }
        }
        ...
        }, 1000); // เพิ่ม delay เป็น 1 วินาทีเพื่อให้ระบบมีเวลาในการประมวลผล
        break;
      }
    }
  });
  ...
  socket.on('disconnect', () => {
    for (let roomName in rooms) {
        if (rooms[roomName].players[socket.id]) {
            const wasLeader = rooms[roomName].players[socket.id].isLeader;
            delete rooms[roomName].players[socket.id]; // ลบผู้เล่นที่ออกจากห้อง
  ...
  setTimeout(async () => {
    try {
      ...
      if (category && category !== "all") {
        // ถ้าหมวดหมู่ถูกเลือกที่ไม่ใช่ "all"
        words = await Word.aggregate([
          { $match: { category: category } },
          { $sample: { size: 2 } }
        ]);
      } else {
        // กรณีเลือกหมวดหมู่ "all"
        // สุ่มหมวดหมู่หนึ่งก่อน แล้วดึงคำจากหมวดหมู่นั้น
        const categories = await Word.distinct('category');
  ...
  socket.on('sendMessage', ({ message, playerId }) => {
    ...
    for (let roomName in rooms) {
      if (rooms[roomName].players[playerId]) {
        const playerName = rooms[roomName].players[playerId].name; // ดึงชื่อผู้เล่น
        
        // เก็บข้อความในประวัติการแชทของห้อง
        if (!chatHistory[roomName]) {
          chatHistory[roomName] = [];
        }
        chatHistory[roomName].push({ playerName, message });
  
        console.log(`Broadcasting message to room: ${roomName}`);
        io.to(roomName).emit('updateMessage', { playerId, message, playerName }); // ส่งชื่อผู้เล่นไปด้วย
        break;
      }
    }
  });
...
const handlePlayerVotedOut = (room, playerOut) => {
  ...
  if (playerOut === gameState[room].spy) {
    console.log(`Player ${playerOut} is Spy, Agents win.`);
    const spyName = rooms[room].players[gameState[room].spy].name;
    io.to(room).emit('gameOver', { result: 'win', room, spyName }); // Agents win
    rooms[room].isGameStarted = false;
    resetlive(room);
  ...
  if (alivePlayers.length === 2) {
    console.log("Only 2 players left, Spy wins.");
    const spyName = rooms[room].players[gameState[room].spy].name;
    io.to(room).emit('gameOver', { result: 'spyWin', room, spyName });
    rooms[room].isGameStarted = false;
    resetlive(room);
    delete gameState[room];
    return;
